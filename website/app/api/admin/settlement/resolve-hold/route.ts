...
import { manualResolveHold } from '../../../../../lib/preparerBalances'
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const result = await manualResolveHold(entryId, {
      reason: reason.trim(),
    ...
    console.error('Error manually resolving hold', err)
    return NextResponse.json(
      { error: 'Unable to resolve hold' },
      { status: 500 },
    )
  }
}
